...
Function Main()
    ...
    While lLoop
        ...
        WHILE ISDIGIT(nNota) = .F.
            QOUT("")
            ACCEPT "Insira aqui o valor da nota (entre 0 e 10): " to nNota

            WHILE Val(nNota) < 0 .or. Val(nNota) > 10
                QOUT("")
                ACCEPT "Valor invalido, digite aqui o valor da nota (entre 0 e 10): " to nNota
...
Static Function MEDIA(nI , nSoma)
    ...
    nMedia := nSoma / nI
    QOUT("A media e:" , Alltrim(STR(nMedia)))
...
Static Function MAIORNOTA(aNotas)
    ...
    aSort(aNotas)
    QOUT("A maior nota foi:" , Alltrim(STR(aNotas[Len(aNotas)])))
...
Static Function MENORNOTA(aNotas)
    ...
    aSort(aNotas)
    QOUT("A menor nota foi:" , Alltrim(STR(aNotas[1])))
